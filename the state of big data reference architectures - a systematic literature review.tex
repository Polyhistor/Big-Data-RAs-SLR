...
The platform search capabilities varied, but our search strategy remained uniform for most parts. For instance, if a platform did not support wildcards ( like asterisk ), we just searched twice for the singular and plural version of the word. The reproducible search for the chosen databases is as follows: 

\begin{itemize}
    \item 'big data' AND ( 'reference architecture*' OR 'architecture*' OR 'model*' OR 'metamodel*' ) 
\end{itemize}

The reason we included architecture, model, and metamodel is due to the fact that these terms may have been used interchangeably, and an architecture that is at the abstraction level of an RA, might have been called just an architecture. Therefore it was critical for us to firmly define these terms and then categorize studies based on these labels. 

The search was limited to 2020 to 2022, as the work of Ataei et al \cite{AtaeiACIS} covered the years 2016-2020, and only to researches that has been published in English. Despite the previous effort to collect evidence on the topic, we still did a quick check on the years 2016 to 2020 to make sure no research is left out. The first limit was imposed because more contemporary researches focused on the facilitation of big data system development in light of failure rates, and the second limit was imposed to increase specificity. 
